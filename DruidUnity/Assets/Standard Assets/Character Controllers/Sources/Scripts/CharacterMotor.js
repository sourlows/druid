...
class CharacterMotorMovement {
	var pushPower : float = 2f;

	// The maximum horizontal speed when moving
	var maxHorizontalSpeed : float = 10.0;
	public var maxForwardSpeed : float = 10f;
	public var maxBackwardsSpeed : float = 10f;
...
private function UpdateFunction () {
	...
	var oldHVelocity : Vector3 = new Vector3(velocity.x, 0, 0);// %%velocity.z);
	movement.velocity = (tr.position - lastPosition) / Time.deltaTime;
	var newHVelocity : Vector3 = new Vector3(movement.velocity.x, 0, 0);//%%movement.velocity.z);
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	if (desiredMovementDirection == Vector3.zero)
		...
		var zAxisEllipseMultiplier : float = (desiredMovementDirection.z > 0 ? movement.maxForwardSpeed : movement.maxBackwardsSpeed) / movement.maxHorizontalSpeed;
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * movement.maxHorizontalSpeed;
		return length;
	}
}
